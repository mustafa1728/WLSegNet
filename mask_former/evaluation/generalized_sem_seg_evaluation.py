...
class GeneralizedSemSegEvaluator(SemSegEvaluator):
    ...
    def __init__(
        ...
        *,
        ...
        self.vis_multiplier = vis_multiplier
    ...
    def process(self, inputs, outputs):
        ...
        """
        pred_save_dir = os.path.join(self._output_dir, "pred_vis")
        if "cls" in self.dataset_name:
            pred_save_dir = os.path.join(pred_save_dir, self.dataset_name.split("_")[-2] + self.dataset_name.split("_")[-1] )
        ...
        for input, output in zip(inputs, outputs):
            ...
            # print(input["file_name"], self.input_file_to_gt_file[input["file_name"]])

            if self.sal_dir is not None:
                sal_path = os.path.join(self.sal_dir, os.path.basename(input["file_name"]).replace(".jpg", ".png"))
                # print(sal_path)
                sal = cv2.imread(sal_path, 0)
                sal = sal > 125
                # print(np.unique(sal), sal.shape, pred.shape)
                pred = pred * sal
            ...
            if_saved = cv2.imwrite(os.path.join(pred_save_dir, input["file_name"].split("/")[-1]).replace(".jpg", ".png"), self.vis_multiplier*pred)
            # print(if_saved, os.path.join(pred_save_dir, input["file_name"].split("/")[-1]), pred.shape)
