...
class ZeroShotMaskFormer(MaskFormer):
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.CLIP_ADAPTER.PROMPT_LEARNER == "instance_learnable":
            adapter_cls = MaskFormerInstanceClipAdapter
        else:
            adapter_cls = MaskFormerClipAdapter
        clip_adapter = adapter_cls(
    ...
    def forward(self, batched_inputs):
        ...
        if self.cfg.MODEL.CLIP_ADAPTER.PROMPT_LEARNER == "instance_learnable":
            img_feats = self.clip_adapter.normalize_feature(outputs["pred_logits"]).mean(0)
            img_feats = img_feats.clone().detach()
            text_features = self.clip_adapter.get_text_features(class_names, img_feats)
        else:
