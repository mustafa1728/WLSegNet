...
class InstanceLearnablePromptExtractor(LearnablePromptExtractor):
    def __init__(self, prompt_dim: int, prompt_shape: Tuple[int, int], img_feat_dim: int, instance_weight):
        super().__init__(prompt_dim, prompt_shape)
        # intermediate_dim = int(np.sqrt(img_feat_dim * self.prompt_dim))
        intermediate_dim =  4 * self.prompt_dim
        self.intermediate_dim = intermediate_dim
        # print("text prompt wabble: ", img_feat_dim, intermediate_dim, self.prompt_dim)
        self.img_tokenizer = nn.Sequential(
                nn.Linear(img_feat_dim, intermediate_dim),
                nn.ReLU(),
                nn.Linear(intermediate_dim, self.prompt_dim),
                nn.ReLU(),
        )
        self.instance_weight = instance_weight
        

    def forward(self, noun_list: List[str], clip_model: nn.Module, img_feat):
        if not self._buffer_init:
            raise RuntimeError(
                f"Buffer of {self.__class__.__name__} is not initialized"
            )
        ...
        img_tokens = self.img_tokenizer(img_feat)
        # print(img_feat.shape, self.intermediate_dim, img_tokens.shape)
        # print(prefix.shape, suffix.shape, img_tokens.shape, img_feat.shape)
        # prefix += img_tokens
        # suffix += img_tokens
        ...
        embeddings = torch.stack(
            [
                torch.cat(
                    [prefix, self.noun_bucket[noun], suffix]
                    + [self.pad_signal.expand(77 - length, -1)]
                ) + self.instance_weight * img_tokens
                for noun, length in zip(noun_list, lengths)
            ]
        )  # cls,77,512
        indices = torch.Tensor(lengths).long().to(embeddings.device) - 1
