...
class ClipAdapter(nn.Module):
    ...
    def forward(self, image: torch.Tensor, text: List[str], target=None, **kwargs):
        image = self._preprocess_image(image, **kwargs)
        text_feature = self.get_text_features(text)  # k,feat_dim
        image_features = self.get_image_features(image)
        

        save_dir = "./mask_former/output_feats_tsne_old"
        os.makedirs(save_dir, exist_ok=True)
        count = 0
        while os.path.exists(os.path.join(save_dir, "{}.npy".format(count))):
            count += 1
        save_dict = {
            "text": text,
            "img_feat": image_features,
            "txt_feat": text_feature,
        }
        if target is not None:
            save_dict["target"] =  target
        np.save(os.path.join(save_dir, "{}.npy".format(count)), save_dict)
...
class InstanceClipAdapter(ClipAdapter):
    def forward(self, image: torch.Tensor, text: List[str], target=None, **kwargs):
        image = self._preprocess_image(image, **kwargs)
        image_features = self.get_image_features(image)
        image_features_prot = image_features.mean(0)
        text_feature = self._get_text_features(text, image_features_prot)  # k,feat_dim

        save_dir = "./mask_former/output_feats_tsne_our"
        os.makedirs(save_dir, exist_ok=True)
        count = 0
        while os.path.exists(os.path.join(save_dir, "{}.npy".format(count))):
            count += 1
    ...
    def _get_text_features(self, noun_list: List[str], img_feat):
        if not self.prompt_learner.with_trainable_params:
            ...
            if len(left_noun_list) > 0:
                left_text_features = self.prompt_learner(
                    left_noun_list, self.clip_model, img_feat
                )
                self.text_feature_buffer.update(
                    {
                        noun: text_feature
            ...
            text_features = self.prompt_learner(noun_list, self.clip_model, img_feat)
            self.text_feature_buffer.update(
                {
                    noun: text_feature.detach()
                    for noun, text_feature in zip(noun_list, text_features)
...
class MaskFormerInstanceClipAdapter(InstanceClipAdapter):
    def __init__(
        self,
        clip_model_name: str,
        prompt_learner: PromptExtractor,
    ...
    def forward(
        ...
        if isinstance(image, list):
            ...
            image_features = self.get_image_features(image)
        text_feature = self.get_text_features(text, image_features.mean(0))  # k,feat_dim
        return self.get_sim_logits(text_feature, image_features), valid_flag
    ...
    def get_text_features(self, noun_list: List[str], img_feat):
        object_text_features = self._get_text_features(noun_list, img_feat.mean(0))
        non_object_text_features = (
            self.non_object_embedding
            / self.non_object_embedding.norm(dim=-1, keepdim=True)
        )
