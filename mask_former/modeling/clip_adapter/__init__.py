...
from .adapter import ClipAdapter, MaskFormerClipAdapter, PerPixelClipAdapter, InstanceClipAdapter, MaskFormerInstanceClipAdapter
...
def build_prompt_learner(cfg):
    ...
    elif cfg.PROMPT_LEARNER == "instance_learnable":
        prompt_learner = InstanceLearnablePromptExtractor(
            prompt_dim=cfg.PROMPT_DIM,
            prompt_shape=cfg.PROMPT_SHAPE,
            img_feat_dim=cfg.PROMPT_DIM,
            instance_weight=cfg.INSTANCE_WEIGHT
        )
        if cfg.PROMPT_CHECKPOINT != "":
            checkpoint = torch.load(cfg.PROMPT_CHECKPOINT, map_location="cpu")["model"]
            missing, unexpected = prompt_learner.load_state_dict(
