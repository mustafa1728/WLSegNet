...
from .modeling.clip_adapter import (
    ...
    InstanceLearnablePromptExtractor
)
...
class ProposalClipClassifier(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        elif cfg.MODEL.CLIP_ADAPTER.PROMPT_LEARNER == "instance_learnable":
            prompt_learner = InstanceLearnablePromptExtractor(
                prompt_dim=cfg.MODEL.CLIP_ADAPTER.PROMPT_DIM,
                prompt_shape=cfg.MODEL.CLIP_ADAPTER.PROMPT_SHAPE,
                img_feat_dim=cfg.MODEL.CLIP_ADAPTER.PROMPT_DIM,
                instance_weight=cfg.MODEL.CLIP_ADAPTER.INSTANCE_WEIGHT
            )
            clip_cls = InstanceClipAdapter
        else:
    ...
    def forward(self, batched_inputs):
        ...
        logits = self.clip_adapter(images, class_names, target=target)
